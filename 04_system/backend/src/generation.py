...
def get_client():
    ...
    if client is None:
        ...
        base_url = os.environ.get("OPENAI_BASE_URL", "https://ai3x-opengoogle.hf.space/v1")
        client = OpenAI(
            api_key=api_key,
            base_url=base_url
        )
...
def configure_client(api_key, base_url=None):
    global client
    client = OpenAI(
        api_key=api_key,
        base_url=base_url or os.environ.get("OPENAI_BASE_URL", "https://ai3x-opengoogle.hf.space/v1")
    )
    return client
...
def call_api(prompt, model, max_tokens_response=4096):
    try:
        ...
        print(f"[DEBUG] call_api: base_url={api_client.base_url}, model={model}")
        ...
        logger.warning(f"API rate limit error: {e}. May need to wait longer before retrying.")
        return None, 0, 0
        ...
        logger.error(f"An unknown error occurred when calling the API: {e}")
        return None, 0, 0
...
def generator(text: str, status: str, model_name: str, models=None, tokenizers=None) -> Tuple[Any, int, int]:
    ...
    if models and tokenizers and TORCH_AVAILABLE:
    ...
    # If using an API model
        ...
        while retry_count < max_retries:
            try:
                from web_demo.trace_runner import running_status
                if running_status.get("should_stop"):
                    return "", 0, 0
            except ImportError:
                pass
