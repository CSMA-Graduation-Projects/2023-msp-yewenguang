...
try:
    ...
    _max_mem = """ + str(512 * 1024 * 1024) + """
    if _platform.system() != 'Windows':
        _resource.setrlimit(_resource.RLIMIT_AS, (_max_mem, _max_mem))
        _resource.setrlimit(_resource.RLIMIT_DATA, (_max_mem, _max_mem))
        if _platform.uname().system != 'Darwin':
            _resource.setrlimit(_resource.RLIMIT_STACK, (_max_mem, _max_mem))
except Exception:
    pass

import builtins as _builtins
_original_exit = _builtins.exit
_original_quit = _builtins.quit
_builtins.exit = None
_builtins.quit = None

import os as _guarded_os, shutil as _guarded_shutil, subprocess as _guarded_subprocess
for _module in [_guarded_os, _guarded_shutil, _guarded_subprocess]:
    for _attr in ['kill', 'system', 'putenv', 'remove', 'removedirs', 'rmdir', 'setuid',
                  'fork', 'forkpty', 'killpg', 'rename', 'renames', 'truncate', 'replace',
                  'unlink', 'fchmod', 'fchown', 'chmod', 'chown', 'chroot', 'lchflags',
                  'lchmod', 'lchown', 'rmtree', 'move', 'Popen']:
        if hasattr(_module, _attr):
            setattr(_module, _attr, None)
...
def execute_code_and_capture_prints_last(
    ...
    full_code_to_execute = SANDBOX_GUARD_CODE + "\n\n" + preprocessed_code_string + "\n\n" + HELPER_CODE_TEMPLATE
